...
namespace Base_Function.BLL_MEDICAL_RECORD_GRADE
{
    ...
    /// 主观评分用户控件
    /// </summary>
    public partial class ucfrmMainGradeRepartDoctor : UserControl
    {
        ...
        public string strText = "3";//医生评分模块标识为3
        ...
        public ucfrmMainGradeRepartDoctor()
        {
            InitializeComponent();
        }

        public ucfrmMainGradeRepartDoctor(ArrayList buttonRights)
        {
            try
            {
                InitializeComponent();
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            if (checkBox1.Checked == true && checkBox7.Checked == true)
            {
                ...
                {
                    this.c1FlexGrid1.Cols["全院医疗分值"].Visible = false;
                    //this.c1FlexGrid1.Cols["全院护理分值"].Visible = false;
                }
            }
        ...
        public void SetFenzhi(double values)
        {
            ...
            if (strZhongMoDoc == "1")
            {
                c1FlexGrid1[c1FlexGrid1.RowSel, "医生医疗分值"] = values;
            }
            if (strYunXingDoc == "2")
            {
                c1FlexGrid1[c1FlexGrid1.RowSel, "医生医疗分值"] = 100 - values;
            }
        }
        ...
        public void SetFenzhiNurse(double values)
        {
            ...
            //if (strZhongMoDoc == "1")
            //{
            //    c1FlexGrid1[c1FlexGrid1.RowSel, "医生护理分值"] = values;
            //}
            //if (strYunXingDoc == "2")
            //{
            //    c1FlexGrid1[c1FlexGrid1.RowSel, "医生护理分值"] = 100 - values;
            //}
        }
        ...
        private void frmMainGradeRepart_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                strUserName = App.UserAccount.UserInfo.User_name.ToString();
                this.GetDoctor();
                cboxSick.Text = strSectionName;//科室传完值后禁止编辑
                cboxSick.Enabled = false;
                cbbDoctorr.Text = strUserName;//管床医生传完值后禁止编辑
                cbbDoctorr.Enabled = false;
                //        }
                //    }
                //}
        ...
        private void c1FlexGrid1_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (c1FlexGrid1.HitTest(e.X, e.Y).Row >= 1)//看是否在信息行里面
                {
                    ...
                    if (strZhongMoDoc == "1")
                    {
                        if (string.IsNullOrEmpty(c1FlexGrid1[c1FlexGrid1.RowSel, "医生医疗分值"].ToString()))
                        {
                        ...
                        //if (string.IsNullOrEmpty(c1FlexGrid1[c1FlexGrid1.RowSel, "医生护理分值"].ToString()))
                        //{
