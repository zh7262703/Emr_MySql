...
{
    public partial class ucTempratureDataLoad_ai : UserControl
    {
        private ParsingFileTLMB _parsingFile = new ParsingFileTLMB();
        ...
        public ucTempratureDataLoad_ai(InPatientInfo info, string tfilename, string tlbfilename)
        {
            InitializeComponent();
            _tlmbFileName = tlbfilename;

            _parsingFile.ParsingXml(_tlmbFileName);
            _parsingFile.CreateWinControls(panel3);
        ...
        private void btnSure_Click(object sender, EventArgs e)
        {
            ...
            if (txtTemper.Text.Trim() != "")
            {
                ...
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, SelectTime, ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL));
            }
            ...
            foreach (object strval in lbEvents.Items)
            {
                ...
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, SelectTime, ctype, l_val, Convert.ToDateTime(Operater_start_time), tempetureDataComm.TEMPLATE_NORMAL));
                ...
                if (strval.ToString().Contains("手术"))
                {
                    ...
                    string sql_operation = "insert into t_job_temp(PID,OPERATE_TYPE,OPERATE_TIME,PATIENT_ID,age) values('" + pid + "','" + operater_type + "',to_timestamp('" + operation_startTime + "','syyyy-mm-dd hh24:mi:ss')," + pids_id + "," + Age + ")";
                    string sql_operations = "insert into t_job_temp(PID,OPERATE_TYPE,OPERATE_TIME,PATIENT_ID,age) values('" + pid + "','" + operater_types + "',to_timestamp('" + operation_endTime + "','syyyy-mm-dd hh24:mi:ss')," + pids_id + "," + Age + ")";
        ...
        private void refleshData()
        {
            ...
            try
            {
                ...
                foreach (DataRow trow in ds.Tables[0].Rows)
                {
                    ...
                    else if (trow["VALTYPE"].ToString() == "辅助呼吸")
                    {
                    ...
                    else if (trow["VALTYPE"].ToString() == "腋温")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    else if (trow["VALTYPE"].ToString() == "口温")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    else if (trow["VALTYPE"].ToString() == "肛温")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    if (trow["VALTYPE"].ToString() == "腋温复测")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    else if (trow["VALTYPE"].ToString() == "口温复测")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    else if (trow["VALTYPE"].ToString() == "肛温复测")
                    {
                        cbTemperType.Text = "";
                        txtTemper.Text = trow["T_VAL"].ToString();
                    }
                    ...
                    {
                        lbEvents.Items.Add(trow["T_VAL"].ToString());
                    }
                ...
                foreach (DataRow trow in ds2.Tables[0].Rows)
                {
                    ...
                    {
                        txtHeight.Text = trow["T_VAL"].ToString();
                    }
        ...
        private void ucTempratureDataLoad_ai_Load(object sender, EventArgs e)
        {
            /*
            ...
            this.pictureBox1.Height = cm.MaxHeight;
            */
        ...
        {
            pictureBox1.Location = new Point((panel4.Width - pictureBox1.Width) / 2, 0);
        }
    }
}
