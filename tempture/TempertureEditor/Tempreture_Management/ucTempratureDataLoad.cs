...
namespace TempertureEditor.Tempreture_Management
{
    public partial class ucTempratureDataLoad : UserControl
    {
        ...
        public ucTempratureDataLoad(InPatientInfo info, string tfilename)
        {
            InitializeComponent();
            ...
            //初始始化cboTime选项数据
            cboTime.Items.Clear();
            string[] times = tempetureDataComm.GetTemperatureWriteTime(tempetureDataComm.TEMPLATE_NORMAL);
        ...
        public void loadTreeRefresh()
        {
            ...
            DataTable dt = App.GetDataSet(string.Format("select aa.VALTYPE_TIME from t_temperature_record aa where " +
                  "aa.valtype ='操作事件' and (aa.t_val like '出院%' or aa.t_val like '死亡%') and patient_id={0} and template_type='{1}'  order by aa.VALTYPE_TIME asc", tPatInfo.Id.ToString(), tempetureDataComm.TEMPLATE_NORMAL)).Tables[0];
            outTime = null;
            if (dt.Rows.Count > 0)
            {
        ...
        private void cboPageIndex_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboPageIndex.SelectedIndex != -1 && this.cboPageIndex.Items.Count > 0)
            {
                ...
                //模板赋值
                tempetureDataComm.GetPageContentByPageObj(tPatInfo, ref currentPage, (cboPageIndex.SelectedIndex + 1).ToString(), outTime,ref cm);
        ...
        private void btnSure_Click(object sender, EventArgs e)
        {
            ...
            sqls.Add("delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                     " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and  MEASURE_TIME=to_date('" + SelectTime.ToString("yyyy-MM-dd HH:mm") + "','yyyy-MM-dd hh24:mi')");
            ...
            if (txtTemper.Text.Trim() != "")
            {
                ...
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, SelectTime, ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            }
            ...
            foreach (object strval in lbEvents.Items)
            {
                ...
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, SelectTime, ctype, l_val, Convert.ToDateTime(Operater_start_time), tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            ...
            lsql = "delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                           " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and to_char(MEASURE_TIME,'yyyy-MM-dd')='" + SelectTime.ToString("yyyy-MM-dd") + "' and VALTYPE='" + ctype + "'";
            sqls.Add(lsql);
            if (cmbBp.Text.Trim() != "")
            {
                l_val = cmbBp.Text;
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            }
            ...
            lsql = "delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                    " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and to_char(MEASURE_TIME,'yyyy-MM-dd')='" + SelectTime.ToString("yyyy-MM-dd") + "' and VALTYPE='" + ctype + "'";
            sqls.Add(lsql);
            if (cmbUrineAmount.Text.Trim() != "")
            {
                l_val = tempetureDataComm.ConvertUrineAmountToDBString(cmbUrineAmount.Text.Trim(), rdInHospital.Checked, rdOperation.Checked, txtUrineHour.Text.Trim(), ckUrine.Checked, ckPGZL.Checked);
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL));
            }
            */
            ...
            lsql = "delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                           " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and to_char(MEASURE_TIME,'yyyy-MM-dd')='" + SelectTime.ToString("yyyy-MM-dd") + "' and VALTYPE='" + ctype + "'";
            sqls.Add(lsql);
            if (txtAllergyHistory.Text.Trim() != "")
            {
                l_val = txtAllergyHistory.Text;
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            }

            
            DateTime dtWeekStart, dtWeekEnd;
            //自定义名称（修改前，应删除本页日期内的所有自定义名称，保证本周只能有一个自定义名称
            ctype = "自定义名称";
            lsql = "delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                           " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and measure_time between to_date('" + currentPage.Starttime + 
                           "','yyyy-MM-dd hh24:mi:ss') and to_date('" + currentPage.Endtime + "','yyyy-MM-dd hh24:mi:ss') and VALTYPE='" + ctype + "'";
            sqls.Add(lsql);
            if (txtCustomName.Text.Trim() != "")
            {
                l_val = txtCustomName.Text;
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            }
            ...
            lsql = "delete from t_temperature_record where patient_id=" + tPatInfo.Id.ToString() +
                           " and template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "' and to_char(MEASURE_TIME,'yyyy-MM-dd')='" + SelectTime.ToString("yyyy-MM-dd") + "' and VALTYPE='" + ctype + "'";
            sqls.Add(lsql);
            if (txtCustomContent.Text.Trim() != "")
            {
                l_val = txtCustomContent.Text;
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, SelectTime, tempetureDataComm.TEMPLATE_NORMAL, measureRemark, valCategory));
            }
            /*
            //皮试-阳性
            ...
            if (txtSkinPlus.Text.Trim() != "")
            {
                l_val = txtSkinPlus.Text + "+" + dateTimeSkinPlus.Value.ToString("yyyy-MM-dd HH:mm"); ;
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, dateTimeSkinPlus.Value, tempetureDataComm.TEMPLATE_NORMAL));
            }
            ...
            if (txtSkinMinus.Text.Trim() != "")
            {
                l_val = txtSkinMinus.Text + "_" + dateTimeSkinMinus.Value.ToString("yyyy-MM-dd HH:mm");
                sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, Convert.ToDateTime(SelectTime.ToString("yyyy-MM-dd")), ctype, l_val, dateTimeSkinMinus.Value, tempetureDataComm.TEMPLATE_NORMAL));
            }
            */
            #endregion
        ...
        private bool checkEventTimeValid()
        {
            ...
            if (!tempetureDataComm.GetDateTimeRangeByMeasure(dtMeasure, dtpAddEventTime.Value, ref dtStart, ref dtEnd, tempetureDataComm.TEMPLATE_NORMAL))
            {
        ...
        private void TemperClear()
        {
            ...
            //尿量
            cmbUrineAmount.Text = "";
            this.ckUrine.Checked = false;
            this.ckPGZL.Checked = false;
            this.rdUnSelected.Checked = true;
            this.rdInHospital.Checked = false;
            this.rdOperation.Checked = false;
            this.txtUrineHour.Text = "";
            */
            //引流量
            ...
            //皮试(阳性)
            txtSkinPlus.Text = "";
            this.dateTimeSkinPlus.Value = Convert.ToDateTime(this.dateTimePicker_Select.Value.ToString("yyyy-MM-dd ") + cboTime.Text);

            //皮试(阴性)
            txtSkinMinus.Text = "";
            this.dateTimeSkinMinus.Value = Convert.ToDateTime(this.dateTimePicker_Select.Value.ToString("yyyy-MM-dd ") + cboTime.Text);
            */
            //自定义名称
            txtCustomName.Text = "";
        ...
        private void refleshData()
        {
            try
            {
                string sql = "select * from T_TEMPERATURE_RECORD t where t.measure_time=to_date('" +
                               ...
                               " and t.template_type='" + tempetureDataComm.TEMPLATE_NORMAL + "'";
                ...
                foreach (DataRow trow in ds2.Tables[0].Rows)
                {
                    ...
                    {
                        string tmp = trow["T_VAL"].ToString();
                        //                 0|1|2|3|4|5   |6|7|8|
                    ...
                    */
                    ...
                    else if (trow["VALTYPE"].ToString() == "过敏史")
                    {
                        txtAllergyHistory.Text = trow["T_VAL"].ToString();
                    }
                    else if (trow["VALTYPE"].ToString() == "自定义内容")
                    {
                        txtCustomContent.Text = trow["T_VAL"].ToString();
                    }
                    /*
                    else if (trow["VALTYPE"].ToString() == "皮试-阳性")
                    {
                        string[] strs = trow["T_VAL"].ToString().Split('+');
                        txtSkinPlus.Text = strs[0];
                        dateTimeSkinPlus.Value = Convert.ToDateTime(strs[1]);
                    }
                    else if (trow["VALTYPE"].ToString() == "皮试-阴性")
                    {
                        string[] strs = trow["T_VAL"].ToString().Split('_');
                        txtSkinMinus.Text = strs[0];
                        dateTimeSkinMinus.Value = Convert.ToDateTime(strs[1]);
                    }
                    */
                }
                //按周显示数据
        ...
        private bool enableInputByInHospitalTime(DateTime dtInHostpital, DateTime dtEnableTime)
        {
            ...
            tempetureDataComm.GetDateTimeRangeByMeasure(dtEnableTime, dtInHostpital, ref dtStart, ref dtEnd, tempetureDataComm.TEMPLATE_NORMAL);
        ...
        {
            DateTime dtWrite = tempetureDataComm.GetInsertDateTime(dt, tempetureDataComm.TEMPLATE_NORMAL);
        ...
        private void ucTempratureDataLoad_Load(object sender, EventArgs e)
        {
