...
namespace Base_Function.BLL_MEDICAL_RECORD_GRADE
{
    ...
    public partial class ucfrmMainGradeRepart : UserControl
    {
        ...
        /// </summary>
        string strRole_tyep = "";
        public string strText = "1";//全院评分类型标识1
        /// <summary>
        /// 运行病历评分
        /// </summary>
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            if (checkBox2.Checked == true)
            {
                strZhongMoDoc = "1";
                querySQL =
                 " select distinct a.id as 编号,a.pid as 住院号,(select section_name from t_sectioninfo where sid= a.section_id) as 科室, a.patient_name as 患者姓名, to_char(a.in_time, 'yyyy-MM-dd HH24:mi') as 入院日期, to_char(a.die_time, 'yyyy-MM-dd HH24:mi') as 出院日期, a.Sick_Doctor_Name as 管床医生, " +
            ...
            if (checkBox1.Checked == true)
            {
                strYunXingDoc = "2";
                querySQL =
              " select distinct a.id as 编号,a.pid as 住院号,(select section_name from t_sectioninfo where sid= a.section_id) as 科室, a.pid as 住院号, a.patient_name as 患者姓名, to_char(a.in_time, 'yyyy-MM-dd HH24:mi') as 入院日期, to_char(a.die_time, 'yyyy-MM-dd HH24:mi') as 出院日期, a.Sick_Doctor_Name as 管床医生, " +
            ...
            else
            {
                ...
                DataRow[] dr_list = dt.Select("全院医疗分值 is not null or 科室医疗分值 is not null or 医生医疗分值 is not null");
            ...
            if (checkBox1.Checked == true && checkBox7.Checked == true)
            {
                ...
                {
                    this.c1FlexGrid1.Cols["全院医疗分值"].Visible = false;
                    //this.c1FlexGrid1.Cols["全院护理扣分值"].Visible = false;
                }
            }
        ...
        public void SetFenzhi(double values)
        {
            if (strZhongMoDoc == "1")
            {
                c1FlexGrid1[c1FlexGrid1.RowSel, "全院医疗分值"] = values;
            }
            if (strYunXingDoc == "2")
            {
                c1FlexGrid1[c1FlexGrid1.RowSel, "全院医疗分值"] = values;
        ...
        public void SetFenzhiNurse(double values)
        {
            //if (strZhongMoDoc == "1")
            //{
            //    c1FlexGrid1[c1FlexGrid1.RowSel, "全院护理分值"] = values;
            //}
            //if (strYunXingDoc == "2")
            //{
            //    c1FlexGrid1[c1FlexGrid1.RowSel, "全院护理分值"] = values;
        ...
        private void frmMainGradeRepart_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //                            cbbPFType.Enabled = false;//禁止编辑评分类型
                //                            cbbPFType.SelectedItem = "科室";
                ...
                //strSelectHistoryPF = "1";//历史评分判断默认设置为1，就是默认选中
        ...
        private void c1FlexGrid1_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (c1FlexGrid1.HitTest(e.X, e.Y).Row >= 1)//看是否在信息行里面
                {
                    ...
                    if (strZhongMoDoc == "1")
                    {
                        if (string.IsNullOrEmpty(c1FlexGrid1[c1FlexGrid1.RowSel, "全院医疗分值"].ToString()))
                        {
                        ...
                        //if (string.IsNullOrEmpty(c1FlexGrid1[c1FlexGrid1.RowSel, "全院护理分值"].ToString()))
                        //{
                    ...
                    if (strYunXingDoc == "2")
                    {
                        ...
                        //if (string.IsNullOrEmpty(c1FlexGrid1[c1FlexGrid1.RowSel, "全院护理扣分值"].ToString()))
                        //{
                        //    pingfenNurseMenuItem.Enabled = true;
                        //}
                        //else
