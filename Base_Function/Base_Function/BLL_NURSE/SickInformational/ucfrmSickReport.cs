...
{
    public partial class ucfrmSickReport : UserControl
    {
        //frmSickReport fsrt = new frmSickReport();
        public ucfrmSickReport()
        {
            InitializeComponent();
        }
        UserRights userRights = new UserRights();
        //权限设置
        public ucfrmSickReport(ArrayList buttonRights)
        {
            InitializeComponent();
            //查询
        ...
        private void ucfrmSickReport_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void SetContent(string data)
        {
            try
            {
                ...
                {
                    cfgcentent.Rows[0].Visible = true;
                    this.cfgcentent.DataSource = dt.DefaultView;
                    cfgcentent.Cols[2].Width = 63;
                ...
                {
                    cfgcentent.DataSource = dt.DefaultView;
                    cfgcentent.Cols[2].Width = 63;
                    cfgcentent.Cols[3].Width = 100;
                    cfgcentent.Cols[4].Width = 350;
                    cfgcentent.Cols[5].Width = 350;
                    ...
                    cfgcentent.Rows[0].Visible = false;
                    cfgcentent.Cols["id"].Visible = false;//设置ID隐藏
                    cfgcentent.Cols["id"].AllowEditing = false;
        ...
        private void btnDay_Click(object sender, EventArgs e)
        {
            ...
            {
                string baiban = "白班";
                AddSickTeport addsick = new AddSickTeport(this, baiban);
                addsick.ShowDialog();
            }
        ...
        private void cfgcentent_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                Point p = cfgcentent.PointToClient(Cursor.Position);
                if (cfgcentent.HitTest(e.X, e.Y).Row >= 1)//看是否在信息行里面
                {
                    DelectUpdateMenuStrip.Show(cfgcentent, p);
                }
            }
        }
        //打印
        private void buttonX1_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = App.GetDataSet("select * from SICKREPORT_VIEW");
                DataSet ds1 = App.GetDataSet("select actiontype,bed_no,patient_name,daywork,remark,sick_area_name "+
                    "from t_handovers_record t join t_In_Patient p on t.pid=p.pid join T_SICKAREAINFO s on t.sid=s.said ");
                SickReportPrint frmsickPrint = new SickReportPrint(ds,ds1);
                frmsickPrint.Show(); 
            }
            catch (Exception ee)
            {
