...
namespace Base_Function.BASE_COMMON
{
    public class printTemper
    {
        ...
        private int gridRows = 45;//表格行数:一个大格5个数值
        private int gridRowsSpace = 14;
        private int bottomRows = 10;//表格下面竖线到的行数
        ...
        private void printHeader()
        {
            ...
            if (isNewheader)
            {
                ...
                graph.DrawString("体 温 单", titleFont, brush, new RectangleF(headerRec.Left, headerRec.Top - headerspace + 10 + headerRec.Height / 4, headerRec.Width, headerRec.Height / 3), textFormat);
                ...
                string[] headlist = new string[] { "登记号", "科室", "床号", "姓名", "性别", "年龄", "入院日期", "住院号" };
                #region 注释
                ...
                for (int i = 0; i < headlist.Length; i++)
                {
                    ...
                    {//姓名
                        ...
                        this.graph.DrawString(itemtext, userFont, brush, headerRec.Left + gridColsWidth * (6 * i - 6) +10, headerRec.Top - headerspace + headerRec.Height / 6 * 4 + 6 - 25);
                    }
                    else if (i == 5)
                    {//年龄
                        //延安市妇幼保健院，页眉年龄特殊处理
                    ...
                    {//性别
                        itemtext = headlist[i] + ":" + user[headlist[i] + ":"];
                        this.graph.DrawString(itemtext, userFont, brush, headerRec.Left + gridColsWidth * (6 * i - 6) + 25, headerRec.Top - headerspace + headerRec.Height / 6 * 4 + 6 - 25);
                    }
                    else if (i == 1)
                    {//科室
                    ...
                    {//床号
                        ...
                        //string itemtext = headlist[i] + ":" + user[headlist[i] + ":"];
                        this.graph.DrawString(itemtext, userFont, brush, headerRec.Left + gridColsWidth * (6 * i - 8) + 50, headerRec.Top - headerspace + headerRec.Height / 6 * 4 + 6 - 25);
                    }
                    else if (i == 6)
                    {//入院日期
                        itemtext = headlist[i] + ":" + user[headlist[i] + ":"];
        ...
        {
            //第 {0} 周(页)
            graph.DrawString(string.Format("第 {0} 页", this.pageIndex), new Font("宋体", 12), brush, 330, 950);//1044
        }
        ...
        {
            int leftwidth = 100;
            for (int i = 0; i < gridCols; i++)  //竖线
            {
                if (i % 6 == 0)
                {
                    blackPen.Width = 2f;
                    graph.DrawLine(blackPen, mainRec.Left + leftwidth + i * hourWidth, mainRec.Top - gridRowsSpace, mainRec.Left + leftwidth + i * hourWidth, mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + bottomRows * bottomRosSpace + rHeight - 100);//25一行
                    blackPen.Width = 1f;
                }
                else
                {
                    ...
                    if ((i - 3) % 6 == 0)
                    {
                        ...
                        graph.DrawLine(blackPen, mainRec.Left + leftwidth + i * hourWidth,mainRec.Top + (topRows - 1) * topRowSpace - gridRowsSpace,
                        //mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + bottomRosSpace * 4 - 10 - 50 + 14,
                        mainRec.Left + leftwidth + i * hourWidth,
                        mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + bottomRosSpace * 1 + rHeight);
                    }
                    else 
                    {
                        graph.DrawLine(blackPen, mainRec.Left + leftwidth + i * hourWidth, mainRec.Top + topRows * topRowSpace - gridRowsSpace, mainRec.Left + leftwidth + i * hourWidth, mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + rHeight);
                    }
                }
            }
        ...
        private void printString()
        {
            ...
            string[] strTemper = new string[] { "41", "40", "39", "38", "37", "36", "35", "34" };
            string[] strPulse = new string[] { "160", "140", "120", "100", "80", "60", "40", "20" };
            ...
            for (int i = 0; i < topRows; i++)
            {
                ...
                //else
                if (i==topRows-1)
                    this.graph.DrawString(strTopCaption[i], nineFont, brush, new RectangleF(mainRec.Left, mainRec.Top + i * topRowSpace - 14 + 2, 100, topRowSpace + gridRowsSpace), textFormat);
                else
            ...
            this.graph.DrawString("脉\n搏",
                nineFont, brush, new RectangleF(mainRec.Left, mainRec.Top + topRows * topRowSpace + 2, 50, 2 * gridRowsSpace), textFormat);
            //this.graph.DrawString("(次/分)", 
            //    nineFont, brush, new RectangleF(mainRec.Left, mainRec.Top + topRows * topRowSpace, 50, 2* gridRowsSpace), textFormat);
            this.graph.DrawString("180",
                tenFont, brush, new RectangleF(mainRec.Left, mainRec.Top + topRows * topRowSpace + 20, 50, 3 * gridRowsSpace), textFormat);
            //this.graph.DrawString("脉\n搏\n次/分", nineFont, brush, new RectangleF(mainRec.Left + 20, mainRec.Top + topRows * topRowSpace, 35, 5 * gridRowsSpace), textFormat);体温\n

            this.graph.DrawString("摄\n氏",
                nineFont, brush, new RectangleF(mainRec.Left + 50, mainRec.Top + topRows * topRowSpace + 2, 50, 2 * gridRowsSpace), textFormat);
            //this.graph.DrawString("(°C)", 
            //    nineFont, brush, new RectangleF(mainRec.Left + 55, mainRec.Top + topRows * topRowSpace, 50, 2 * gridRowsSpace), textFormat);
            this.graph.DrawString("42",
                tenFont, brush, new RectangleF(mainRec.Left + 50, mainRec.Top + topRows * topRowSpace + 20, 50, 3 * gridRowsSpace), textFormat);
            ...
            for (int i = 0; i < 7; i++)
            {
                this.graph.DrawString("上午", eightFont, brush, new RectangleF(mainRec.Left + 100 + i * 6 * gridColsWidth, mainRec.Top + (topRows - 1) * topRowSpace - gridColsWidth, gridColsWidth * 3, topRowSpace), textFormat);
                this.graph.DrawString("下午", eightFont, brush, new RectangleF(mainRec.Left + 100 + i * 6 * gridColsWidth + 3 * gridColsWidth, mainRec.Top + (topRows - 1) * topRowSpace - gridColsWidth, gridColsWidth * 3, topRowSpace), textFormat);
                for (int j = 0; j < 6; j++)
                {
                    int timepoit = 0;
                    timepoit = j * 4 + 2;
                    ...
                    if (timepoit == 2 || timepoit == 18 || timepoit == 22)
                        //2,18,22点变红
                        this.graph.DrawString(hour.ToString(), eightFont, redBrush, new RectangleF(mainRec.Left + 100 + i * 6 * gridColsWidth + j * gridColsWidth - 1, mainRec.Top + (topRows - 1) * topRowSpace + 2, gridColsWidth + 2, topRowSpace), textFormat);
                    else
                        this.graph.DrawString(hour.ToString(), eightFont, brush, new RectangleF(mainRec.Left + 100 + i * 6 * gridColsWidth + j * gridColsWidth - 1, mainRec.Top + (topRows - 1) * topRowSpace + 2, gridColsWidth + 2, topRowSpace), textFormat);
             ...
             * 体温
             */
            int strArrTop = mainRec.Top + topRows * topRowSpace + (5 + 3) * gridRowsSpace;
            for (int i = 0; i < strTemper.Length; i++) //strTemper.Length
            {
                this.graph.DrawString(strTemper[i], tenFont, brush, new RectangleF(mainRec.Left + 50, strArrTop - 5, 50, gridRowsSpace), textFormat);
            ...
            strArrTop = mainRec.Top + topRows * topRowSpace + (5 + 3) * gridRowsSpace;
            for (int i = 0; i < strPulse.Length; i++)
            {
                this.graph.DrawString(strPulse[i], tenFont, brush, new RectangleF(mainRec.Left, strArrTop - 5, 50, gridRowsSpace), textFormat);
                strArrTop = strArrTop + 5 * gridRowsSpace;
            }
            ...
            //this.graph.DrawString("血压(mmHg)", nineFont, brush, new RectangleF(mainRec.Left, strArrTop, 100, bottomRosSpace), textFormat);
            for (int i = 0; i < strBottomCaption.Length; i++)
            {
                // if (i == 0 || i == 1)
                //{
                //this.graph.DrawString(strBottomCaption[i], nineFont, brush, new RectangleF(mainRec.Left+65, strArrTop, 20, bottomRosSpace), textFormat);
                //}
                //else
                //{
        ...
        private void printLine()
        {
            ...
            for (int i = 0; i <= topcount; i++)
            {
                ...
                if (i == topcount)
                    this.graph.DrawLine(blackPen, mainRec.Left + 100, mainRec.Top + i * topspace - 14, mainRec.Right, mainRec.Top + i * topspace - 14);
                else
                    this.graph.DrawLine(blackPen, mainRec.Left, mainRec.Top + i * topspace - 14, mainRec.Right, mainRec.Top + i * topspace - 14);
            }
            //体温脉搏表格上方横线
            blackPen.Width = 2f;
            this.graph.DrawLine(blackPen, mainRec.Left, mainRec.Top + 4 * topspace, mainRec.Right, mainRec.Top + 4 * topspace);
            blackPen.Width = 1f;
            ...
            //体温脉搏中间分割线
            this.graph.DrawLine(blackPen, mainRec.Left + 55, mainRec.Top + topRows * topRowSpace, mainRec.Left + 55, mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace);
            //延安妇幼 血压标题中间分割线
            //this.graph.DrawLine(blackPen, mainRec.Left + 55, mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + rHeight, mainRec.Left + 55, mainRec.Top + topRows * topRowSpace + gridRows * gridRowsSpace + rHeight+50);
                ...
                for (int i = 0; i < tempers.Rows.Count; i++)
                {
                    ...
                    if (measureState != "R")
                    {
                        ...
                        if (pulseValue >= 20) //脉搏y坐标
                        {
                            ...
                            PY = ((180 - (float)pulseValue)) / 4 * gridRowsSpace + mainRec.Top + topRows * topRowSpace + 3 * gridRowsSpace;
                            if (i > 0)
                            {
                        ...
                        if (heartValue >= 20)  //心率
                        {
                            HY = ((180 - (float)heartValue)) / 4 * gridRowsSpace + mainRec.Top + topRows * topRowSpace + 3 * gridRowsSpace;
                        ...
                        if (temperValue > 0) //体温y坐标
                        {
                            ...
                            {
                                temperValue = 35;
                                TY = mainRec.Top + topRows * topRowSpace + (42 - (float)temperValue) * 5 * gridRowsSpace + 3 * gridRowsSpace;
                                //this.graph.DrawString("↓", eightFont, Brushes.Blue, new RectangleF(timeX - gridColsWidth / 2 + 5, mainRec.Top + topRows * topRowSpace + gridRowsSpace * 41 - 12, gridColsWidth, gridRowsSpace * 2), textFormat);
                                this.graph.DrawString("不\n升", eightFont, Brushes.Blue, new RectangleF(timeX - gridColsWidth / 2 , mainRec.Top + topRows * topRowSpace + gridRowsSpace * 39 - 12, gridColsWidth, gridRowsSpace * 2), textFormat);
                                //temperStyle = 2; //不升
                            ...
                            {
                                temperStyle = 0;  //高温
                                TY = mainRec.Top + 81+ 3 * gridRowsSpace;
                            }
                            else
                            {
                                TY = mainRec.Top + topRows * topRowSpace + (42 - (float)temperValue) * 5 * gridRowsSpace + 3 * gridRowsSpace;
                            ...
                            {
                                float coolY = mainRec.Top + topRows * topRowSpace + (42 - (float)coolValue) * 5 * gridRowsSpace + 3 * gridRowsSpace;
        ...
        private bool LQROEvent(float evertx, float timeX, string measureState, int hourWidth)
        {
            ...
            RectangleF otherCharRec = new RectangleF(evertx - 7, mainRec.Top + topRows * topRowSpace + (35+3) * gridRowsSpace+1, hourWidth, hourWidth);//延安妇幼事件显示在35刻度下显示
        ...
        {
            RectangleF charRec = new RectangleF(evertx - 7, mainRec.Top + topRows * topRowSpace + gridRowsSpace * 3, hourWidth, hourWidth);//88
        ...
        private void DrawEvent(float evertx, string var, string measure_time)
        {
            ...
            RectangleF charRec = new RectangleF(evertx - 7, mainRec.Top + topRows * topRowSpace + 2 + gridRowsSpace*3, hourWidth, hourWidth);//88
            if (evertx - 7 < mainRec.Left + 688)
            {
                string[] eventTime = var.Split('_');
                DateTime dateEvent;
                ...
                //}else
                if (var.Contains("回家待产") || var.Contains("返院待产") || var.Contains("回家待术") || var.Contains("返院待术"))//不显示时间
                {
                    eventTimeString = eventTime[0];
                }
                else
        ...
        private void printOther()
        {
            ...
            for (int i = 0; i < other.Rows.Count; i++)
            {
                ...
                if (bp_blood != "")
                {
                    ...
                    itemRec.Y = mainTop + bottomRosSpace * 0;
                    Font objfont = new Font("宋体", 7.5f); //7.5f  
                    Font eightFontTemp = new Font("宋体", 9f);
                    itemRec.Width = gridColsWidth * 3 + 2;
                ...
                if (i == 0)
                {
                    ...
                    //    RectangleF itemRec = textRec;
                    //    itemRec.X = mainRec.X;
                    //    itemRec.Y = mainTop + bottomRosSpace * 8;
                    //    itemRec.Width = 100;
                                ...
                                if (length > 0)
                                {
                                    ...
                                    else
                                    {
                                        if (surgeryDays != "")
                                        {
                                            ...
                                            surgeryDays = NumberConvertToNoman(j + 1) + "- " + abject.Days.ToString();
                                        }
                                        else
                                        {
