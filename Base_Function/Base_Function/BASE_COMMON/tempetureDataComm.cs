...
namespace TempertureEditor.Tempreture_Management
{
    public class tempetureDataComm
    {
        ...
        public static bool InsertAutoOptEvent(InPatientInfo tPatInfo, DateTime dtEvent, string sEventName, string templateType)
        {
            ...
            sqls.Add(tempetureDataComm.GetInsertSql(tPatInfo, SelectTime, ctype, l_val, dtEvent, templateType));
        ...
        /// <returns></returns>
        /// 
        public static string GetTemplateFileByType(string type)
        {
            string tmbXmlFile;
            if (type == tempetureDataComm.TEMPLATE_NORMAL)
                tmbXmlFile = "TempertureSet_newTable.tmb";
            else if (type == tempetureDataComm.TEMPLATE_BABY)
                tmbXmlFile = "TempertureSet_newTable_baby.tmb";
            else if (type == tempetureDataComm.TEMPLATE_CHILD)
        ...
        public static string GetInsertSql(int Id, string Sick_Bed_Name,  string Sick_Area_Name, string Section_Name, DateTime SelectTime, string cctype, string cval, DateTime cctypeTime, string template)
        {
            string sql = "insert into t_temperature_record(SICK_BED_NAME,SICK_AREA_NAME,SECTION_NAME,TEMPLATE_TYPE,MEASURE_TIME,T_VAL,VALTYPE,PATIENT_ID, VALTYPE_TIME)values('" +
                 Sick_Bed_Name + "','" + Sick_Area_Name + "','" + Section_Name + "','" + template + "',to_date('" +
                SelectTime.ToString("yyyy-MM-dd HH:mm") + "','yyyy-mm-dd hh24:mi'),'" + cval + "','" + cctype + "','" + Id + "',to_date('" +
                cctypeTime.ToString("yyyy-MM-dd HH:mm") + "','yyyy-mm-dd hh24:mi'))";
            return sql;
        }
        /// <summary>
        ...
        public static string GetInsertSql(InPatientInfo inPat, DateTime SelectTime, string cctype, string cval, DateTime cctypeTime, string template)
        {
            string sql = "insert into t_temperature_record(SICK_BED_NAME,SICK_AREA_NAME,SECTION_NAME,TEMPLATE_TYPE,MEASURE_TIME,T_VAL,VALTYPE,PATIENT_ID, VALTYPE_TIME)values('" +
                ...
                SelectTime.ToString("yyyy-MM-dd HH:mm") + "','yyyy-mm-dd hh24:mi'),'" + cval + "','" + cctype + "','" + inPat.Id + "',to_date('" +
                cctypeTime.ToString("yyyy-MM-dd HH:mm") + "','yyyy-mm-dd hh24:mi'))";
            return sql;
        }
        ...
        public static DateTime GetInsertTimeFromRange(DateTime dt, string[] times, string templateType)
        {
            ...
            {
                dtMeasure = Convert.ToDateTime(dt.ToString("yyyy-MM-dd ") + times[i]);
        ...
        public static bool GetDateTimeRangeByMeasure(DateTime dtMeasure, DateTime dtEvent, ref DateTime dtLower, ref DateTime dtUpper, string templateType)
        {
            ...
            if (templateType == TEMPLATE_NORMAL || templateType == TEMPLATE_BABY)
            {
                tsPlus = new TimeSpan(1, 59, 0);
                tsMinus = new TimeSpan(2, 0, 0);
            }
            else if (templateType == TEMPLATE_CHILD)
            {
                tsPlus = new TimeSpan(1, 59, 0);
                tsMinus = new TimeSpan(2, 0, 0);
            }
                                ...
                                foreach (string surgeryStr in surgerys)
                                {
                                    ...
                                    if (surgeryStr.Contains("手术") || surgeryStr.Contains("分娩"))
                                    {
                                        ...
                                        if (abject.Days.ToString() == "0")
                                        {
                                            ...
                                            if (j > 0)
                                                surgeryDays = surgeryDays + "/" + surgery;
                        ...
                        !ds.Tables[0].Rows[i]["VALTYPE"].ToString().Contains("皮试"))
                    {
                        ...
                        else if (cm.GetVDataSetByName(ds.Tables[0].Rows[i]["VALTYPE"].ToString()) != null)
                        {
                            if (ds.Tables[0].Rows[i]["T_VAL"].ToString() != "")
                            {
                                ...
                                tempobj.Val = ds.Tables[0].Rows[i]["T_VAL"].ToString();
                                tempobj.Typename = ds.Tables[0].Rows[i]["VALTYPE"].ToString();
                                if (tempobj.Typename == "尿量")
                                {
                                    string[] strs = tempobj.Val.Split('|');
                                    tempobj.Val = "";
                                    for (int index = 0; index < strs.Length; index++)
                                    {
                                        tempobj.Val += strs[index];
                                    }
                                    if (tempobj.Val.Substring(tempobj.Val.Length - 1) == "/")
                                        tempobj.Val = tempobj.Val.Substring(0, tempobj.Val.Length - 1);
                                    tempobj.Rdatatime = Convert.ToDateTime(ds.Tables[0].Rows[i]["MEASURE_TIME"]).ToString("yyyy-MM-dd HH:mm");
                                }
                                else if (tempobj.Typename == "自定义名称")
                                    tempobj.Rdatatime = "";
        ...
        public static void GetPageContentByPageObj_child(InPatientInfo inPat, ref Page currentPage, string indexPage, DateTime? dtOutHospital, ref Comm cm)
        {
            ...
            try
            {
                ...
                string sql = "select * from t_temperature_record aa where " +
                             "(aa.measure_time between to_date('{0}','yyyy-MM-dd hh24:mi') and " +
                             "to_date('{1}','yyyy-MM-dd hh24:mi') or aa.valtype in ('多或少于出生体重', '出院体重', '疫苗接种日期'))and patient_id={2} and template_type='{3}'  order by aa.VALTYPE_TIME asc";
                ...
                */
                ...
                tempobj_intime.Typename = "入院时间";
                tempobj_intime.Rdatatime = "";
                tempobj_intime.Val = inPat.In_Time.ToString("yyyy-MM-dd HH:mm");
                tempobj_intime.setdataxy(currentPage.Starttime);
                currentPage.Objs.Add(tempobj_intime);
                */
                ...
                */
                            ...
                            if (ds.Tables[0].Rows[i]["T_VAL"].ToString() != "")
                            {
                                ...
                                if (tempobj.Typename == "尿量")
                                {
                                    ...
                                    tempobj.Rdatatime = Convert.ToDateTime(ds.Tables[0].Rows[i]["MEASURE_TIME"]).ToString("yyyy-MM-dd HH:mm");
                                         ...
                                         tempobj.Typename == "出院体重" ||
                                         tempobj.Typename == "多或少于出生体重" ||
                                         tempobj.Typename == "疫苗接种日期")
                                {
                                    tempobj.Rdatatime = "";
                                }
                                else if (tempobj.Typename == "操作事件" && tempobj.Val.Contains("手术"))
                ...
                /*
                 *操作事件 时间转中文处理,同时处理出院时间根据出院事件判断
                 * 出院时间，不对直接从病人基本信息中取，因为一个患者可能多次出入院，
                 * 每个阶段住院体温单的病人基本信息应该独立存储
                */
                ...
                if (dtOutHospital != null)
                {
                    ClsDataObj tempobj_outtime = new ClsDataObj(cm);
                    tempobj_outtime.Typename = "出院日期";
                    tempobj_outtime.Rdatatime = "";
                    tempobj_outtime.Val = ((DateTime)dtOutHospital).ToString("yyyy-MM-dd");
                    tempobj_outtime.setdataxy(currentPage.Starttime);
                    currentPage.Objs.Add(tempobj_outtime);
                }

                // 体重升降显示规则，出生、出院时显示在对应时间点，其他日期的体重升降显示在8：00
                string dayBirthday = "";
                if (dtBirthday != null)
                {
                    dayBirthday = ((DateTime)dtBirthday).ToString("yyyy-MM-dd");
                }

                string dayOutHospital = "";
                if (dtOutHospital != null)
                {
                    dayOutHospital = ((DateTime)dtOutHospital).ToString("yyyy-MM-dd");
                }

                foreach (ClsDataObj obj in currentPage.Objs)
                {
                    if (obj.Typename == "体重升降")
                    {
                        string dayRdatatime = Convert.ToDateTime(obj.Rdatatime).ToString("yyyy-MM-dd");
                        if (dayBirthday == dayRdatatime)
                        {
                            obj.Rdatatime = tempetureDataComm.GetInsertDateTime((DateTime)dtBirthday, tempetureDataComm.TEMPLATE_CHILD).ToString("yyyy-MM-dd HH:mm");
                        }
                        else if (dayOutHospital == dayRdatatime)
                        {
                            obj.Rdatatime = tempetureDataComm.GetInsertDateTime((DateTime)dtOutHospital, tempetureDataComm.TEMPLATE_CHILD).ToString("yyyy-MM-dd HH:mm");
                        }
                        else
                        {
                            obj.Rdatatime = dayRdatatime + " " + "08:00";
                        }
                        obj.setdataxy(currentPage.Starttime);
                    }
                }
            ...
            {
                App.Msg("tempertureDataComm.GetPageContentByPageObj_Child()方法异常!");
            }
